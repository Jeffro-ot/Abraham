...
msgcon = {
    mostwanted = "You are now the most wanted player on the server!",
    killwanted = "You have killed the most wanted player on the server!",
    }
}
...
function onLogin(login, player)
    ...
    player:sendTextMessage(MESSAGE_STATUS_WARNING, config.msgcon.mostwanted, player:getPosition())
end
...
function onKill(player, target)
    ...
    player:sendTextMessage(MESSAGE_STATUS_WARNING, config.msgcon.killwanted, target:getPosition())
end
